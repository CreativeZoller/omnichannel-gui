...
export class FailedMessagesComponent implements OnInit {
  ...
  getData() {
    ...
    this.adminService.getFailedMessages(yyyy + '-' + mm + '-' + dd, yyyy2 + '-' + mm2 + '-' + dd2).subscribe(
      (result) => {
  ...
  private setDataSource() {
    this.displayedColumns = ['Sid', 'Body', 'DateCreated', 'MessageErrorMessage', 'AccountId'];
    this.dataSource = new MatTableDataSource<IFailedMessage>(this.data);
