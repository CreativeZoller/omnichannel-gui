...
@Component({
  selector: 'vt-communications-page',
...
export class CommunicationsPageComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.activatedroute.data.subscribe((data) => {
      ...
      this.dataSource = new MatTableDataSource<Accounts>(this.accountsData);
    });
  }
  ...
  loadOlderMessages() {
    let messageErrors = 0;
    this.accounts.getArchivedMessagesByAccount(this.selectedAccount).subscribe(
      (result) => {
        result.forEach((value) => {
          if (value.ErrorMessage !== '') messageErrors++;
        });
        if (messageErrors === 0) {
          const accountData: Messages[] = result;
          for (const message of this.accountMessages) {
            accountData.push(message);
          }
          this.accountMessages = accountData;
  ...
  loadAccountDetailPage() {
    window.open(`${Constants.dashBoardUrl}showall/showallbridge/${this.selAccount.Source}/${this.selAccount.MasterKey}`, '_blank');
  }
  listenForUpdates() {
